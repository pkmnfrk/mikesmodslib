    ...
    extends ShapedOreRecipe
{
    public FluidRecipe(ResourceLocation group, @Nonnull ItemStack result, CraftingHelper.ShapedPrimer primer)
    {
        ...
        //Mod.logger.debug("Initializing fluid-based recipe to create " + result);
    }
    ...
    {
        //Mod.logger.debug("Does this fluid match? '" + fluid.getFluid().getName() + "x" + fluid.amount + "mB' vs '" + stack + "'");

        if(stack.isEmpty())
        {
            //Mod.logger.debug("Nope, stack is empty");
            return false;
        }

        if(!stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null))
        {
            //Mod.logger.debug("Nope, stack doesn't have capability");
            return false;
        }
        ...
        //Mod.logger.debug("Table: {}", DataUtils.toJson(nbt).toString());
        ...
        if(drained == null || drained.getFluid() != fluid.getFluid() || drained.amount != fluid.amount)
        {
            ...
            //}
            //Mod.logger.debug("Nope, stack couldn't drain fluid: {}", dbg);
            return false;
        }

        //Mod.logger.debug("Yep");
        return true;
    }
    ...
    public boolean matches(InventoryCrafting inventoryCrafting, World world)
    {
        ...
        //Mod.logger.debug("Checking matching for recipe for " + this.getRecipeOutput());
        ...
        for(int i = 0; i < ingredients.size(); i++)
        {
            ...
            if(desiredFluid != null)
            {
                ...
                //Mod.logger.debug("Ingredient: {}", DataUtils.toJson(nbt).toString());

                if(!fluidMatches(desiredFluid, stack))
                {
                    //Mod.logger.debug("Recipe doesn't match because fluid doesn't match");
                    return false;
                }
            }
            else
            {
                if(!ing.apply(stack))
                {
                    //Mod.logger.debug("Recipe doesn't match because {} doesn't match {}", stack, toString(ing));
                    return false;
                }
            }
        }

        //Mod.logger.debug("Recipe matches!");
